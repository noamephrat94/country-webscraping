...
def switzerland(driver, df):
    ...
    yesterday = datetime.now() - timedelta(1)
    d = datetime.strftime(yesterday, '%d')
    deaths = f'https://www.covid19.admin.ch/api/data/{str(year),str(month),str(d)}-kiq8id6u/epidemiologic/epidemiologic-development-death-ch.json'
    cases = f'https://www.covid19.admin.ch/api/data/{str(year),str(month),str(d)}-kiq8id6u/epidemiologic/epidemiologic-development-case-ch.json'
...
def liechtenstein(driver, df):
    ...
    total_cases = total_cases['values'][-2]['total']
    total_deaths = total_deaths['values'][-2]['total']
...
def czech(driver, df):
    ...
    html_source = driver.page_source
    soup = BeautifulSoup(html_source, 'html.parser')
    hospital = soup.find('p', id='count-hospitalization')
    hospital = hospital.text
    deaths = soup.find('p', id='count-dead')
    total_deaths = deaths.text.replace(" ", "").replace("\n", "")
    cases = soup.findAll('div', class_='background--primary text--white pa-10 border--radius-rounded')
    total_cases = cases[0].find('span').attrs.get("data-value", None)
    new_cases = cases[0].find('span', class_='text--sm').attrs.get("data-value-sick-yesterday", None)
...
def israel(driver, df):
    ...
    total_cases = stats[2]
    hospital = stats[5]
    total_deaths = re.sub("[^0-9a-zA-Z]+", "", data[3].text)
    stats = []
    for d in data[1]:
        stats.append(re.sub("[^0-9a-zA-Z]+", "", d.text))
    icu = stats[0]
...
def main():
    ...
    df.to_csv(f'output/output_{today}.csv', index=False)
